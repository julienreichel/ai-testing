...
describe("RulesEditorCompact - Rule Options", () => {
  ...
  it("should handle combined case sensitivity and punctuation options", () => {
    ...
    const caseSensitivePuncIgnoreRule = {
      ...mockStringRule,
      caseSensitive: true,
      respectPunctuation: false,
    };
    expect(testStringRule(caseSensitivePuncIgnoreRule, "SOL.U TI:ON!")).toBe(
      false,
    ); // Wrong case
    expect(testStringRule(caseSensitivePuncIgnoreRule, "sol.u ti:on!")).toBe(
      true,
    ); // Right case, punctuation ignored

    // Test case insensitive + respect punctuation (case unchecked, punctuation checked)
    const caseInsensitivePuncRespectRule = {
      ...mockStringRule,
      caseSensitive: false,
      respectPunctuation: true,
    };
    expect(testStringRule(caseInsensitivePuncRespectRule, "Solution")).toBe(
      true,
