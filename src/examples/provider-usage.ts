...
export async function setupOpenAIProvider(): Promise<void> {
  ...
  if (
    ...
    try {
      ...
      console.log("Cost:", `$${response.cost.totalCost.toFixed(COST_DIGIT)}`);
...
export function useProviderRegistry(): void {
  ...
  console.log(
    "Active Providers after update:",
    registry.getActiveProviders().length,
  );
}
...
export function demonstrateCostEstimation(): void {
  ...
  const text =
    "This is a sample text for token estimation. It should be counted accurately.";
  ...
  const estimatedCost = mockProvider.estimateCost(
    TOKENS_1000,
    TOKENS_500,
    "mock-fast",
  );
  const COST_DIGIT = 6;
  console.log(
    `Cost for 1000 input + 500 output tokens: $${estimatedCost.toFixed(COST_DIGIT)}`,
  );
}
