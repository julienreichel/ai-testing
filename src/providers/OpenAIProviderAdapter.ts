...
export class OpenAIProviderAdapter extends BaseProviderAdapter {
  ...
  async call(request: ProviderRequest): Promise<ProviderResponse> {
    if (!this.validateConfig()) {
      const error = this.createError('auth', 'API key is required for OpenAI provider')
  ...
  private async makeRequest(request: ProviderRequest): Promise<Response> {
    ...
    const body = {
      model: request.model,
      messages,
      temperature: request.temperature ?? OpenAIProviderAdapter.DEFAULT_TEMPERATURE,
    ...
    try {
      const response = await fetch(`${OpenAIProviderAdapter.BASE_URL}/chat/completions`, {
        method: 'POST',
        headers: {
  ...
  private async processResponse(
    ...
    const inputCost = pricing ? (inputTokens / TOKENS_PER_1K) * pricing.inputTokensPer1K : 0
    const outputCost = pricing ? (outputTokens / TOKENS_PER_1K) * pricing.outputTokensPer1K : 0
  ...
  private createError(type: ProviderError['type'], message: string): ProviderError {
    return {
      code: 'PROVIDER_ERROR',
      message,
      type,
  ...
  private createHttpError(status: number, errorData: Record<string, unknown>): ProviderError {
    const message = (errorData.error as { message?: string })?.message || 'Unknown error'
